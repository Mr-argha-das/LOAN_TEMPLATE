...
import { ChevronDown, LogOut, Menu, Phone, Search, User, X } from "lucide-react";
import { CholaLogo } from "@/components/CholaLogo";
import { AuthDialog, type AuthMode } from "@/components/AuthPanel";
import {
  AUTH_CHANGED_EVENT,
  getCurrentUser,
  logoutUser,
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {user ? (
              <div className="hidden items-center gap-2 sm:flex">
                <span className="flex max-w-[140px] items-center gap-1.5 text-[13px] font-semibold text-chola-blue">
                  <User className="h-4 w-4 shrink-0 text-chola-red" />
                  <span className="truncate">{user.fullName}</span>
                </span>
                <button
                  type="button"
                  onClick={() => void signOut()}
                  className="flex items-center gap-1 rounded-full border border-border px-3 py-2 text-[13px] font-semibold text-chola-grey transition-colors hover:border-chola-blue hover:text-chola-blue"
                >
              ...
              <div className="hidden items-center gap-2 sm:flex">
                <button
                  ...
                  className="rounded-full border border-chola-blue px-4 py-2 text-[13px] font-bold text-chola-blue transition-colors hover:bg-chola-blue hover:text-white"
                >
                  Login
                </button>
                <button
                  ...
                  className="rounded-full bg-chola-blue px-4 py-2 text-[13px] font-bold text-white transition-colors hover:bg-chola-blue/90"
                >
                  Register
                </button>
              </div>
      ...
      {open && (
        <div className="max-h-[70vh] overflow-y-auto border-b border-border bg-white lg:hidden">
          <div className="px-4 py-3">
            {user ? (
              <div className="mb-3 flex items-center justify-between gap-3 rounded-md bg-secondary px-3 py-2">
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold text-chola-blue">
                    {user.fullName}
                  </span>
                  <span className="block truncate text-[12px] text-chola-grey">{user.email}</span>
                </span>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="flex shrink-0 items-center gap-1 text-[13px] font-semibold text-chola-blue"
                >
                  <LogOut className="h-4 w-4" /> Logout
                </button>
              </div>
              ...
              <div className="mb-3 grid grid-cols-2 gap-2">
                <button
                  ...
                  className="rounded-full border border-chola-blue py-2 text-[13px] font-bold text-chola-blue"
                >
                  Login
                </button>
                <button
                  ...
                  className="rounded-full bg-chola-blue py-2 text-[13px] font-bold text-white"
                >
                  Register
                </button>
              </div>
